...
describe('ppr-full', () => {
  ...
  if (!isNextDev) {
    describe('HTML Fallback', () => {
      ...
      const pathnames: Array<{ pathname: string; slug: string }> = []
      const patterns: Array<(slug: string) => string> = [
        (slug) => `/fallback/params/${slug}`,
        (slug) => `/fallback/use-pathname/${slug}`,
        (slug) => `/fallback/use-params/${slug}`,
        (slug) => `/fallback/use-selected-layout-segment/${slug}`,
        (slug) => `/fallback/use-selected-layout-segments/${slug}`,
      ]
      for (let i = 0; i < 5; i++) {
        for (const pattern of patterns) {
          const slug = `slug-${String(i).padStart(2, '0')}`
          pathnames.push({ pathname: pattern(slug), slug: slug })
        }
      }

      describe.each(pathnames)('for $pathname', ({ pathname, slug }) => {
        it('should render the fallback HTML immediately', async () => {
          ...
          expect(chunks.static).not.toContain(slug)
          expect(chunks.dynamic).toContain(slug)
